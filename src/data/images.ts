import type { Media } from "@/types/content";
// Temporary generated concept visuals. Replace paths and alt text with approved assets.
export const images = {
  hero: {
    src: "/images/automotive-hero.webp",
    blurDataURL:
      "data:image/webp;base64,UklGRjIAAABXRUJQVlA4ICYAAACQAQCdASoIAAUAA8BgJaQAApz+dxAA/vXrZNBMyplI3zCTKAAAAA==",
    alt: "Серебристый автомобиль в тёмной студии — концептуальная иллюстрация",
    position: "center",
  },
  electric: {
    src: "/images/electric-v2.webp",
    blurDataURL:
      "data:image/webp;base64,UklGRjYAAABXRUJQVlA4ICoAAACQAQCdASoIAAUAA8BgJaQAApexCwAA/XgsNeLV1WrxvRRTDBxxQAAAAAA=",
    alt: "Светлый электрический кроссовер у современной архитектуры — иллюстрация категории",
    position: "center",
  },
  hybrid: {
    src: "/images/automotive-suv.webp",
    blurDataURL:
      "data:image/webp;base64,UklGRjoAAABXRUJQVlA4IC4AAACQAQCdASoIAAUAA8BgJZQCdADZgLAAzj5IER6abNNQ0nUqRomYrwRhENwTAAAA",
    alt: "Серебристый кроссовер в светлой студии — концептуальная иллюстрация",
    position: "center 58%",
  },
  petrol: {
    src: "/images/petrol-v2.webp",
    blurDataURL:
      "data:image/webp;base64,UklGRjIAAABXRUJQVlA4ICYAAACQAQCdASoIAAUAA8BgJaQAAvdoSQAA/t21x8SvoaMJQHNIR4AAAA==",
    alt: "Тёмно-зелёный бензиновый седан с открытой решёткой радиатора — иллюстрация категории",
    position: "center",
  },
  interior: {
    src: "/images/interior-v2.webp",
    blurDataURL:
      "data:image/webp;base64,UklGRjYAAABXRUJQVlA4ICoAAACQAQCdASoIAAUAA8BgJaQAAp0isHAA/un4c6vPnHubJB3rtO07CaAgAAA=",
    alt: "Кожаное сиденье и центральная консоль современного автомобиля — иллюстрация деталей салона",
    position: "center",
  },
  final: {
    src: "/images/road-v2.webp",
    blurDataURL:
      "data:image/webp;base64,UklGRjQAAABXRUJQVlA4ICgAAACQAQCdASoIAAUAA8BgJZwAApZCkeAA/vXp1S7JcSQkB73POYMMbIAA",
    alt: "Бензиновый SUV на горной дороге в сумерках — концептуальная иллюстрация",
    position: "center 60%",
  },
} satisfies Record<string, Media>;
